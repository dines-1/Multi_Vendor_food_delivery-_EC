  ...
  return (
    <div className="admin-page">
        ...
        <div className="table-toolbar">
          ...
          <div className="table-filters">
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: 6 }}>
              <input className="filter-input" placeholder="Search users..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: 200 }} />
              ...
              <tbody>
                {users.map(u => (
                  <tr key={u._id}>
                    ...
                    <td>{u.phone}</td>
                    <td>
                      <span className={`badge ${u.role === 'admin' ? 'badge-purple' : u.role === 'vendor' ? 'badge-info' : 'badge-default'}`}>
                        {u.role}
                      </span>
                    </td>
      ...
      {modal?.type === 'role' && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3>Change Role</h3><button className="modal-close" onClick={() => setModal(null)}><X size={16} /></button></div>
            <div className="modal-body">
              <div className="form-group">
            ...
            <div className="modal-footer">
              <button className="btn btn-outline" onClick={() => setModal(null)}>Cancel</button>
              <button className="btn btn-primary" onClick={() => handleRoleChange(modal.id)}>Update Role</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal-content" style={{ maxWidth: 520 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {detail.orders?.length > 0 && (
                <>
                  <div className="form-label" style={{ marginBottom: 8 }}>Order History</div>
